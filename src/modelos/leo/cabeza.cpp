...
CabezaLeo3D::CabezaLeo3D(const std::string & filename) : Objeto3D(){
  ...
  //Colocamos el pelo
  setHijo(new CuboObj3D());
  hijos[2] -> setTranformacion(_vertex4<float>(Escalado_, 1.1, 0.8, 1.1));
  hijos[2] -> setTranformacion(_vertex4<float>(Translacion_, 0.0, 0.8, -0.05));
  hijos[2] -> setColor(_vertex3<float>(82.0/255.0, 41.0/255.0, 0.0/255.0));
